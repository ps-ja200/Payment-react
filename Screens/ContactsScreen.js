                ...
                {filteredContacts.map((contact, index) => (
                    <View key={index} style={contactsStyles.contactItem}>
                        ...
                        <View style={contactsStyles.contactIcons}>
                            <TouchableOpacity
                                style={contactsStyles.callIcon}
                                onPress={() => navigation.navigate('SendMoney', { name: contact.name, email: contact.email })}
                            >
                                <Image source={require("../images/send_icon.png")} style={contactsStyles.callIcon}/>
                            </TouchableOpacity>
                            <TouchableOpacity
                                style={contactsStyles.messageIcon}
                                onPress={() => navigation.navigate('RequestMoney', { name: contact.name, email: contact.email })}
                            >
...
const contactsStyles = StyleSheet.create({
    ...
    personimage: { width: 45, height: 45, borderRadius: 15 },
    addIcon: { height: 40, width: 40, marginRight: 10 }
});
